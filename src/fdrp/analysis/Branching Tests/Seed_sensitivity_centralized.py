...
"""
This code generates a dataset with the seeds provided in "DATA_SEEDS". I trains the centralized model and logs the results to a excel file.
...
SUMMARY_PATH = Path(r"C:\Users\oskar\OneDrive\Desktop\Seed_test\seed_sensitivity_summary.csv")

PLOT_PATH = Path(r"C:\Users\oskar\OneDrive\Desktop\Seed_test\seed_sensitivity_plot.png")
...
def load_base_config() -> ExperimentConfig:
    return ExperimentConfig(
        ...
        experiment_id="seed_sensitivity_centralized",
    )
...
def config_for_run(
    ...
    cfg.experiment_type = "centralized"
    cfg.experiment_id = f"seed_sensitivity_centralized_seed_{data_seed}"

    cfg.dataset_path = str(dataset_path)
    cfg.test_set_path = str(testset_path)

    # Fixed seeds, so only data generation seed changes
    ...
    cfg.run_suffix = f"seed_{data_seed}"
...
def run_single_seed(data_seed: int) -> Dict[str, Any]:
    ...
    print("\n" + "=" * 80)
    print(f"Running CENTRALIZED seed sensitivity for data_seed={data_seed}")
    print("=" * 80)
...
def seed_sensitivity() -> None:
    ...
    print(f"\nSaved seed sensitivity summary to: {SUMMARY_PATH}")
    ...
    if "f1_global_macro" in df.columns:
        ...
        plt.title("Centralized seed sensitivity")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
